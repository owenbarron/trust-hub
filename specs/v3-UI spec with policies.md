...
**Policies Tab:**
Table showing linked policies:
| Policy | Version | Status | Relationship | Review Due |
Example row: `Asset Management Policy | v2.1 | [Active] | Governs | 03/15/2027`

The **Relationship** column uses descriptive, color-coded labels:
- "Fulfills this control" — `text-green-700 bg-green-50 px-2 py-0.5 rounded text-xs` — means the policy document IS the evidence
- "Governs this control" — `text-blue-700 bg-blue-50 ...` — means separate operational evidence is needed
- "Acknowledgement required" — `text-amber-700 bg-amber-50 ...` — means proof of personnel acknowledgement is needed

If a "fulfills" policy has a document attached, show a small "View document" link inline.
If an "acknowledgement required" policy has no acknowledgement evidence in the current audit, show a subtle red warning icon.
...
### 5. Policies List (`/policies`)

**Not scoped to the audit switcher** — policies are living documents. The page should have a subtle note at the top: "Policies exist across all audit periods." (`text-xs text-gray-400`)

**Table:**
| Column | Width | Content |
|--------|-------|---------|
| ID | 80px | `POL-001` — link to detail |
| Name | flex/grow | Policy name |
| Version | 60px | e.g. "2.1" |
| Status | 100px | Badge: Active (green), Draft (gray), Under Review (yellow), Retired (red) |
| Owner | 140px | Name |
| Review Due | 120px | Date — normal text if future, **red text with `AlertTriangle` icon** if overdue |
| Controls | 80px | Count of linked controls |

Same table styling as controls list.

### 6. Policy Detail (`/policies/[id]`)

**Header:**
```
Breadcrumb: Policies > POL-001
Title: POL-001 Asset Management Policy
[Active] badge    Version 2.1
```

If the policy has a document file: show a "View Document" / "Download" button in the header area (secondary button style).

**Metadata Section:**
Two-column grid similar to control detail:
```
Owner               MJ Eldridge
Status              [Active]
Version             2.1
Approved Date       09/15/2025
Review Due          09/15/2026
Description         Defines the procedures for tracking, classifying, and managing
                    all information system assets...
```

**Linked Controls Section:**
Grouped by relationship type, with clear group headers:

```
┌─────────────────────────────────────────────────────────────────────┐
│ ✓ Fulfills (2 controls)                                            │
│ This policy directly satisfies these controls.                     │
│                                                                    │
│ CTL-071  Asset Management Policy      [In progress]  [Effective]   │
│ CTL-022  Company Policies             [In progress]  [Effective]   │
├─────────────────────────────────────────────────────────────────────┤
│ ⟶ Governs (3 controls)                                             │
│ These controls operate under this policy. Separate evidence needed.│
│                                                                    │
│ CTL-040  Infrastructure Inventory     [In progress]  [Effective]   │
│ CTL-062  Termination Checklist        [In progress]  [Effective]   │
│ CTL-061  User Access Requests         [In progress]  [Effective]   │
├─────────────────────────────────────────────────────────────────────┤
│ ✋ Requires Acknowledgement (1 control)                             │
│ Evidence of personnel acknowledgement is required.                 │
│                                                                    │
│ CTL-029  Policy Acknowledgement       [In progress]  [Effective]   │
└─────────────────────────────────────────────────────────────────────┘
```

Group headers use:
- Fulfills: `text-green-700` with `CheckCircle` icon
- Governs: `text-blue-700` with `ArrowRight` icon
- Requires Acknowledgement: `text-amber-700` with `Hand` icon (or `UserCheck` from Lucide)

Each control row links to the control detail page.

### 7. Request Detail (`/requests/[id]`)
...
### 8. Criteria Coverage Matrix (`/criteria`)
...
- `Grid3x3` — Criteria Matrix
- `LayoutDashboard` — Dashboard
- `FileText` — PDF evidence / Policies nav
- `BookOpen` — Policy document
